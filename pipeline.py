...
def pull_parcels(county_key: str) -> list[dict]:
    ...
    # Try searchByUrl endpoint first (most reliable for filtered searches)
    county_slug = county["name"].replace(" ", "-").replace("'", "-")
    redfin_url = "https://www.redfin.com/county/{}/MD/{}-County/filter/property-type=land,max-price=150000".format(
        county["region_id"], county_slug
    )

    # Endpoint 1: searchByUrl
    try:
        url = "https://redfin-base.p.rapidapi.com/searchByUrl"
        payload = {"url": redfin_url, "limit": 350}
        r = requests.post(url, headers=RAPIDAPI_HEADERS, json=payload, timeout=30)
        log.info(f"  searchByUrl status: {r.status_code}")
        if r.status_code == 200:
            data = r.json()
            parcels = parse_rapidapi_response(data, county["name"])
            if parcels:
                log.info(f"  {len(parcels)} listings via searchByUrl")
                return parcels
    except Exception as e:
        log.warning(f"  searchByUrl failed: {e}")

    time.sleep(1)

    # Endpoint 2: searchProperties
    try:
        url = "https://redfin-base.p.rapidapi.com/searchProperties"
        payload = {
            "location": county["search"],
            "propertyType": "land",
            "maxPrice": int(BUY_BOX["max_price"] * 1.5),
            "limit": 350,
        }
        r = requests.post(url, headers=RAPIDAPI_HEADERS, json=payload, timeout=30)
        log.info(f"  searchProperties status: {r.status_code}")
        if r.status_code == 200:
            data = r.json()
            parcels = parse_rapidapi_response(data, county["name"])
            if parcels:
                log.info(f"  {len(parcels)} listings via searchProperties")
                return parcels
    except Exception as e:
        log.warning(f"  searchProperties failed: {e}")

    time.sleep(1)

    # Endpoint 3: search (GET)
    try:
        url = "https://redfin-base.p.rapidapi.com/search"
        params = {
            "location": county["search"],
            "type": "land",
            "maxPrice": int(BUY_BOX["max_price"] * 1.5),
        }
        r = requests.get(url, headers=RAPIDAPI_HEADERS, params=params, timeout=30)
        log.info(f"  search GET status: {r.status_code}")
        if r.status_code == 200:
            data = r.json()
            parcels = parse_rapidapi_response(data, county["name"])
            log.info(f"  {len(parcels)} listings via search GET")
            return parcels
    except Exception as e:
        log.warning(f"  search GET failed: {e}")
