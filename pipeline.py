...
# ── REDFIN COUNTY IDs ─────────────────────────────────────────────────────────
COUNTIES = {
    "anne_arundel":   {"name": "Anne Arundel",    "region_id": "1311", "region_type": "5"},
    "prince_georges": {"name": "Prince George's",  "region_id": "1325", "region_type": "5"},
    "howard":         {"name": "Howard",            "region_id": "1322", "region_type": "5"},
    "charles":        {"name": "Charles",           "region_id": "1317", "region_type": "5"},
}
...
def pull_parcels(county_key: str) -> list[dict]:
    ...
    try:
        ...
        parcels = []
        for row in reader:
            price_str = str(row.get("PRICE") or "0").replace("$","").replace(",","").strip()
            sqft_str  = str(row.get("SQUARE FEET") or "0").replace(",","").strip()
            lot_str   = str(row.get("LOT SIZE") or "0").replace(",","").strip()
