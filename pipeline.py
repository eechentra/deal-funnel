...
def pull_parcels(county_key: str) -> list[dict]:
    ...
    # /SearchForSale with county location + land type filter
    try:
        params = {
        ...
        r = requests.get(
            "https://realtor-data3.p.rapidapi.com/SearchForSale",
            headers=headers, params=params, timeout=30
        )
        log.info(f"  /SearchForSale status: {r.status_code}")
        if r.status_code == 200:
            ...
            if parcels:
                ...
                return parcels
            log.info(f"  Empty response — keys: {list(data.keys()) if isinstance(data,dict) else type(data)}")
            log.info(f"  Sample: {str(data)[:300]}")
    except Exception as e:
        log.warning(f"  /SearchForSale failed: {e}")

    time.sleep(2)

    # Fallback: no type filter (get all, filter locally)
    try:
        params = {
            "location": county["search"],
            "sort":     "best_match",
        ...
        log.info(f"  /SearchForSale (no filter) status: {r.status_code}")
        if r.status_code == 200:
            data = r.json()
            parcels = parse_rapidapi_response(data, county["name"])
            log.info(f"  {len(parcels)} listings returned")
